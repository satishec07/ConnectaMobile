...
class TestMoreDevice_Plan(BaseClass):
    def test_SinglePlanTwoDevice_GoToPlanPage(self):
        ...
        wait = WebDriverWait(self.driver, 10)
        wait.until(expected_conditions.element_to_be_clickable((By.XPATH, "//input[@value='2']")))
        # time.sleep(4)
        ...
        self.driver.execute_script("window.scrollBy(0, 2200);")
        wait = WebDriverWait(self.driver, 10)
        wait.until(expected_conditions.presence_of_element_located((By.ID, "fname")))
        # self.driver.execute_script("window.scrollBy(0, 1200);")
        ...
        print("Before clicked on getAgree_TC()", Dplan.getAgree_TC().is_selected())
        wait = WebDriverWait(self.driver, 10)
        wait.until(expected_conditions.element_to_be_clickable((By.ID, "agreed_doc")))
        Dplan.getAgree_TC().click()
        print("After clicked on getAgree_TC()", Dplan.getAgree_TC().is_selected())
        ...
        wait = WebDriverWait(self.driver, 10)
        wait.until(expected_conditions.element_to_be_clickable((By.ID, "validate_amt")))
        Dplan.getPlace_Order().click()
        wait = WebDriverWait(self.driver, 14)
        wait.until(expected_conditions.presence_of_element_located((By.CSS_SELECTOR, "h2.mainTittle.cart.cloudSafe")))
